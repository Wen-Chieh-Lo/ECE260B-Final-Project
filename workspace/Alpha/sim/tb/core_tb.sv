// Core Verification - SystemVerilog
...
`ifdef SIM_INTERACTIVE
		...
		//########################################################################
		$display("");
		$display(">>> Interactive mode. Type: qkp, norm, vp, reset, quit, help");
		$display("");
		forever begin
			$write("core_tb> ");
			...
			if (cmd == "qkp") begin
				...
				Start1Cyc;
				WaitCoreDone;
				PrintVerifyHeader("Matrix Multiplication", "Test 1   |  PMEM =?= QMEM * Transpose(KMEM)", "", "");
				VerifyPMEM(0);
				...
				Start1Cyc;
				WaitCoreDone;
				PrintVerifyHeader("Matrix Multiplication + Result Normalization", "Test 2    |  PMEM  =?=  Norm (QMEM * Transpose(KMEM))",
				...
				Start1Cyc;
				WaitCoreDone;
				PrintVerifyHeader("Full MULT-NORM-MULT Pipeline", "Test 3   |  PMEM =?= QMEM * Transpose(Norm)",
