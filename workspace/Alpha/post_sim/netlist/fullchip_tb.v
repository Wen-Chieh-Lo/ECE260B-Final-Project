...
module fullchip_tb;
	...
	initial begin
		...
		for (t = 0; t < total_cycle; t = t+1)
			for (q = 0; q < col; q = q+1) begin
				...
				vn_c1[t][q] = 0;
				for (k = 0; k < pr; k = k+1) begin
					vn_c0[t][q] = vn_c0[t][q] + V_T[t][k] * N_est_c0[k][q];
					vn_c1[t][q] = vn_c1[t][q] + V_T[t][k] * N_est_c1[k][q];
				end
			end
		for (c = 0; c < col; c = c+1) golden_col[c] = 7 - c + 1;
	...
	initial begin
		...
		repeat(3) tick0;  // PMEM read latency
		for (q0 = 0; q0 < total_cycle; q0 = q0+1) begin
			row0 = q0;
		...
		// ===== Test 2 =====
		reset0 = 1; repeat(10) tick0; reset0 = 0; repeat(3) tick0;
	...
	initial begin
		...
		repeat(3) tick1;  // PMEM read latency
		for (q1 = 0; q1 < total_cycle; q1 = q1+1) begin
			row1 = q1;
		...
		// ===== Test 2 =====
		reset1 = 1; repeat(10) tick1; reset1 = 0; repeat(3) tick1;
