...
module sfp_row (clk, reset, acc, div, fifo_ext_rd, sum_in, sum_out, sfp_in, sfp_out, sfp_out_valid);
  ...
  // SFP_THRESHOLD: row energy threshold for sparsity gating.
  // Define via +define+SFP_THRESHOLD=<value>; default = 0 (no gating).
`ifndef SFP_THRESHOLD
  `define SFP_THRESHOLD 0
`endif
  localparam [bw_psum+3:0] SFP_THRESHOLD_VAL = `SFP_THRESHOLD;
  ...
  wire [bw_psum+3:0] sum_2core_gated;
  ...
  // per-row energy and normalization skip flag
  wire [bw_psum+3:0] row_energy;
  wire               skip_norm_row;
  reg                skip_norm;
  reg                skip_norm_nxt;
  ...
  // row energy Si = Σj |Qi×Kj|
  assign row_energy    = sum8_out;
  assign skip_norm_row = (row_energy < SFP_THRESHOLD_VAL);
  // when row is below threshold, treat normalization divisor as zero for divider's internal zero-detect
  assign sum_2core_gated = skip_norm ? { (bw_psum+4){1'b0} } : sum_2core;
...
`else
  ...
  div_longdiv #(.bw_psum(bw_psum+4), .out_shift(out_shift)) div1 (
    ...
    .divisor(sum_2core_gated), .out(div_out1), .done()
  );
  div_longdiv #(.bw_psum(bw_psum+4), .out_shift(out_shift)) div2 (
    ...
    .divisor(sum_2core_gated), .out(div_out2), .done()
  );
  div_longdiv #(.bw_psum(bw_psum+4), .out_shift(out_shift)) div3 (
    ...
    .divisor(sum_2core_gated), .out(div_out3), .done()
  );
  div_longdiv #(.bw_psum(bw_psum+4), .out_shift(out_shift)) div4 (
    ...
    .divisor(sum_2core_gated), .out(div_out4), .done()
  );
  div_longdiv #(.bw_psum(bw_psum+4), .out_shift(out_shift)) div5 (
    ...
    .divisor(sum_2core_gated), .out(div_out5), .done()
  );
  div_longdiv #(.bw_psum(bw_psum+4), .out_shift(out_shift)) div6 (
    ...
    .divisor(sum_2core_gated), .out(div_out6), .done()
  );
  div_longdiv #(.bw_psum(bw_psum+4), .out_shift(out_shift)) div7 (
    ...
    .divisor(sum_2core_gated), .out(div_out7), .done()
  );
`endif
  ...
  always @(*) begin
    ...
    if (acc) begin
      sum_q_nxt = sum8_out;
      // capture per-row sparsity decision when accumulating this row
      skip_norm_nxt = skip_norm_row;
    end
