// step1: QK product only. Trimmed from core_tb (remove Phase 2 sfp, Phase 3 VN).
...
module step1_tb;
  ...
  initial begin
    ...
    for (c = 0; c < col; c = c+1)
      golden_col[c] = 7 - c;
    $display("QK phase verification (checking pmem content)\n");
    ...
    #0.5 clk = 1'b1;
    for (q = 0; q < total_cycle; q = q+1) begin
      #0.5 clk = 1'b0; pmem_add = pmem_add+1;
      #0.5 clk = 1'b1;
      row = q;
